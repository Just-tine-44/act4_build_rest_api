...
userRouter.get("/users", async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        if (!allUsers) {
            res.status(StatusCodes.NOT_FOUND).json({ message: `No users at this time..` })
            return
        }
...
userRouter.post("/login", async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        if (!user) {
            res.status(StatusCodes.BAD_REQUEST).json({ error: `No user exists with the email provided..` })
            return
        }
